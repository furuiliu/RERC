...
def eval(prediction_file, gold_file):
    ...
    metrics = [{'em': 0, 'f1': 0, 'prec': 0, 'recall': 0,
        ...
        'joint_em': 0, 'joint_f1': 0, 'joint_prec': 0, 'joint_recall': 0} for _ in range(4)]
    Sum = [0,0,0,0]
    for dp in gold:
        cur_id = dp['_id']
        idx = Q_type.get(dp['type'],0)
        Sum[idx] += 1
        can_eval_joint = True
        if cur_id not in prediction['answer']:
            ...
            em, prec, recall = update_answer(
                metrics[idx], prediction['answer'][cur_id], dp['answer'])
        if cur_id not in prediction['sp']:
            print('missing sp fact {}'.format(cur_id))
            can_eval_joint = False
        else:
            sp_em, sp_prec, sp_recall = update_sp(
                metrics[idx], prediction['sp'][cur_id], dp['supporting_facts'])
        # 
        if cur_id not in prediction['evidence']:
            ...
            evi_em, evi_prec, evi_recall = update_evi(
                metrics[idx], prediction['evidence'][cur_id], dp['evidences'])
        ...
        if can_eval_joint:
            ...
            metrics[idx]['joint_em'] += joint_em
            metrics[idx]['joint_f1'] += joint_f1
            metrics[idx]['joint_prec'] += joint_prec
            metrics[idx]['joint_recall'] += joint_recall
    ...
    for i in range(4):
        ...
        for k in metrics[i].keys():
            metrics[i][k] = round(metrics[i][k]/N*100, 2)
        print(Types[i])
        print(json.dumps(metrics[i], indent=4))
