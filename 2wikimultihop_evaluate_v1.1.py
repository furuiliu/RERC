...
if __name__ == '__main__':
    ...
    for f in onlyfiles:
        try:
            metrics.append(eval(f, "data_ids/dev.json","data_ids/id_aliases.json"))
        except KeyError as e:
            print(e)


    keys = ['em', 'f1', 'pr', 're', 'sp_em', 'sp_f1', 'sp_pr', 'sp_re', 'ev_em', 'ev_f1', 'ev_pr', 'ev_re','jt_em', 'jt_f1', 'jt_pr', 'jt_re']
    print('\t' + '\t'.join(keys))

    temp = 'ep%02d\t' + '\t'.join(['%.4f']*16)
    # print(temp)
    best_iter = -1
    best_em = -1
