    ...
    def __iter__(self):
        ...
        while True:
            ...
            for k in range(len(cur_batch)):
                ...
                if 'evidences' in cur_batch[k]:
                    y[k] = Q_type.get(cur_batch[k]["type"],0)
                                                                                                                                                                                                               
                    subject = [cur_batch[k]["evidences"][0][0]]
                    for i in range(1,len(cur_batch[k]["evidences"])): 
                        if cur_batch[k]["evidences"][i][0] not in cur_batch[k]["evidences"][i-1]:
                            subject.append(cur_batch[k]["evidences"][i][0])
                    input_str = " ".join(query_tokens)
                    if len(subject)>0:
                        ans = " ".join(tokenizer.tokenize(subject[0]))
                        y1s[k],y1e[k] = find_subpos(input_str,ans)
                    if len(subject)>1:
                        ans = " ".join(tokenizer.tokenize(subject[1]))
                        y2s[k],y2e[k] = find_subpos(input_str,ans)
    ...
    for batch in tqdm(dataloader):
        ...
        for i in range(t.shape[0]):                                                                                                                                                                        
            ...
            if int(t[i]) < 2:
                ...
                answer_dict[cur_id] = {'Type':int(t[i]),'Subject':[ans],'answers':[ans,ori_ans1],
                ...
                answer_dict[cur_id] = {'Type':int(t[i]),'Subject':[ans1,ans2],'answers':[ans1,ori_ans1,ans2,ori_ans2],
                                        'spans':[[l1,r1,y1s,y1e],[l2,r2,y2s,y2e]]}
    writer.write('T-acc: {:.3f}\t R_f1:{:.3f}\n'.format(sum(t_acc)/len(t_acc),sum(s_acc)/len(s_acc)))
...
def set_config():
    ...
    # Required parameters
    parser.add_argument("--name", type=str, default='Subject')
...
if __name__ == "__main__":
    ...
    if not args.do_eval:                                                                                                                                                                                                  
        model.train()
        for epc in trange(args.epochs, desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(Loader, desc="Iteration")):
                ...
                if global_step % 1000 == 0 and step>0:
                    loss_log = 'total:{}\t Subject1:{}\t Subject2:{}'.format(tr_loss/step,r1_loss/step,r2_loss/step)
